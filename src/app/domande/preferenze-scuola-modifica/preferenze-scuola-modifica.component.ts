...
@Component({
  selector: 'app-preferenze-scuola-modifica',
  templateUrl: './preferenze-scuola-modifica.component.html',
  styleUrls: ['./preferenze-scuola-modifica.component.css']
})
export class PreferenzeScuolaModificaComponent implements OnInit {
  codAnagraficaGraduatoria: string;
  idGraduatoria : string;
  idDomandaIscrizione : string;
  ordinePreferenza : string;
  codScuola : string;
  descScuola : string;
  indirizzo : string;
  idTipoFrequenza : string;
  ...
  statoFuturo: string = "";
  descStatoFuturo: string = "";
  idStatoFuturo: number = 0;
 ...
 /**
  ...
  get statoDomanda(): any {
    return this.domandaNido.statoDomanda
  }
  ...
  ngOnInit() {
    ...
    this.codAnagraficaGraduatoria = this.route.snapshot.queryParamMap.get('codAnagraficaGraduatoria');
    this.idGraduatoria = this.route.snapshot.queryParamMap.get('idGraduatoria');
    this.idDomandaIscrizione = this.route.snapshot.queryParamMap.get('idDomandaIscrizione');
    this.ordinePreferenza = this.route.snapshot.queryParamMap.get('ordinePreferenza');
    this.codScuola = this.route.snapshot.queryParamMap.get('codScuola');
    this.descScuola = this.route.snapshot.queryParamMap.get('descScuola');
    this.indirizzo = this.route.snapshot.queryParamMap.get('indirizzo');
    this.idTipoFrequenza = this.route.snapshot.queryParamMap.get('idTipoFrequenza');
    this.codTipoFrequenza = this.route.snapshot.queryParamMap.get('codTipoFrequenza');
    this.descTipoFre = this.route.snapshot.queryParamMap.get('descTipoFre');
    this.idStatoScu = this.route.snapshot.queryParamMap.get('idStatoScu');
    this.codStatoScu = this.route.snapshot.queryParamMap.get('codStatoScu');
    this.descStatoScu = this.route.snapshot.queryParamMap.get('descStatoScu');
    this.dtStato = this.route.snapshot.queryParamMap.get('dtStato');

    console.log('idGraduatoria: ' + this.idGraduatoria)

    this.initStatoFuturo(this.codStatoScu);
  ...
  private initStatoFuturo(statoAttuale: string): void{

      if(statoAttuale =="AMM"){
        this.statoFuturo = "PEN";
        this.descStatoFuturo = "In graduatoria";
        this.idStatoFuturo = 1;
      }
      if(statoAttuale == "RIN"){
        this.statoFuturo = "AMM";
        this.descStatoFuturo = "Ammesso";
        this.idStatoFuturo = 3;
      }
      if(statoAttuale == "RIN_AUTO"){
        this.statoFuturo = "PEN";
        this.descStatoFuturo = "Ammesso";
        this.idStatoFuturo = 3;
      }
  ...
  confermaOnClick() {
    ...
    this.domandaService.modificaStatoScuola(+this.idGraduatoria, +this.idStatoScu, this.idStatoFuturo).subscribe(
      res => {
        let errmsg = ("Operazione completata")
        if(res.messages[0]){ errmsg = (`${errmsg}. ${res.messages[0]}`)}

        if(res.messages[1]){ errmsg = (`${errmsg}. ${res.messages[1]}`)}
        
        alert(errmsg);
        
        this.location.back();},
      err => {
        if (err.code = "VAL-GRA-011") {
          alert("Operazione non eseguibile perchè non ci sono posti liberi")
        } else 
        console.log("errore modifica stato", err)
      }
