...
@Component({
  selector: 'app-dettaglio-domanda-preventive',
  templateUrl: './dettaglio-domanda-preventive.component.html',
  styleUrls: ['./dettaglio-domanda-preventive.component.css']
})
export class DettaglioDomandaPreventiveComponent implements OnInit {
  globalError: boolean = false;
  errorConcorrenza: boolean = false;
  ...
  ngOnInit() {
    ...
    this.radioFormFunzioneVariazioneSocSan.addControl('valPun', new FormControl('',
      [Validators.required]));
    this.radioFormFunzioneVariazioneSocSan.addControl('valPunRed', new FormControl('',
      [Validators.required]));
  ...
  get disabledButtonConfirmSocSan() {
    ...
    } else if (this.radioSelectedSocSan === 'ACC-RED-CON') {
  ...
  public returnTestataIstruttoria() {
    this.router.navigate(['istruttoria/verifiche/preventive/caching']);
  }
  ...
  getVerifichePreventive() {
    ...
    this.loginService.getUtenteLoggato()
      .subscribe({
        next: (data: UserInfo) => {
          const user = data;
          this.istruttoriaService.getElencoCondizioni(ordineScuola, user.codFisc, 'P')
            .subscribe({
    ...
    this.istruttoriaService.funzioneVariazione(
      ...
      error => {
        if (error && error.title && error.title.includes('VAL-IST-006')) {
          ...
          this.errorConcorrenza = true;
          this.getStoricoVariazioni(this.idDomanda, this.codiceCondizionePunteggio);
          setTimeout(() => {
            this.errorConcorrenza = false;
          }, 9000);
        }
      }
    );
  ...
  funzioneSocioSanitaria() {
    let ricorrenza;
    let codFunVar;
    if ((this.radioSelectedSocSan === 'ACC-PUN-PRIO' || this.radioSelectedSocSan === 'ACC-PUN' || this.radioSelectedSocSan === 'ACC-RED-CON')) {
      codFunVar = 'VAL';
      ricorrenza = 1;
    } else if (this.radioSelectedSocSan === 'RIF-SOCIO-SAN'
